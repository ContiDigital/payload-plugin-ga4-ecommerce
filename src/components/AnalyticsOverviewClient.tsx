...
const ChartMetricToggle: React.FC<{
  ...
  return (
    <label
      style={{ alignItems: 'center', display: 'inline-flex', fontSize: '0.8rem', gap: '0.35rem' }}
    >
      <input
        aria-label={label}
        checked={checked}
  ...
  return (
    ...
    >
      ...
      >
        ...
        <p
          style={{ color: 'var(--theme-elevation-600)', fontSize: '0.8rem', margin: '0.25rem 0 0' }}
        >
          {subheading}
        </p>
      </header>
...
export const AnalyticsOverviewClient: React.FC<Props> = ({
  ...
  const [landingPageReport, setLandingPageReport] = useState<null | ReportResult>(null)
  ...
  const nextUseCacheRef = useRef(true)
  ...
  useEffect(() => {
    ...
    const run = async () => {
      setIsLoading(true)
      const useCacheForRequest = nextUseCacheRef.current
      nextUseCacheRef.current = true
      const cachePreference = useCacheForRequest ? {} : { useCache: false }
      ...
      setLandingPageReport(
        landingPagesResult.status === 'fulfilled' ? landingPagesResult.value : null,
      )
  ...
  const eventRows = useMemo(() => {
    ...
    const hasNonZeroMetrics = (
      metrics: Partial<Record<MetricKey, number>> | undefined,
    ): boolean => {
    ...
    const displayRows =
      prioritizedTrackedRows.length > 0
        ? [...prioritizedTrackedRows, ...additionalRows]
        : additionalRows
  ...
  const topPageRows = useMemo(
    () =>
      (topPagesReport?.rows ?? []).map((row, index) => ({
        cells: [
          row.dimensionValue || '(not set)',
          formatInteger(row.metrics.views),
          formatInteger(row.metrics.visitors),
        ],
        key: `${row.dimensionValue}-${index}`,
      })),
  ...
  const landingPageRows = useMemo(
    () =>
      (landingPageReport?.rows ?? []).map((row, index) => ({
        cells: [
          row.dimensionValue || '(not set)',
          formatInteger(row.metrics.sessions),
          formatInteger(row.metrics.visitors),
  ...
  return (
    <AnalyticsErrorBoundary>
      <section style={{ padding: '1rem 0' }}>
        ...
        {health ? (
          ...
          >
            <p style={{ fontSize: '0.82rem', margin: 0 }}>
              Cache: {health.cache.enabled ? `enabled (${health.cache.strategy})` : 'disabled'} |
              Rate limiting: {health.rateLimit.enabled ? 'enabled' : 'disabled'}
            </p>
            <p
              style={{
                color: 'var(--theme-elevation-600)',
            ...
            >
              Tracked events:{' '}
              {health.events.trackedEventNames.length
                ? health.events.trackedEventNames.join(', ')
                : 'all events'}
            </p>
          </div>
        ) : null}
            ...
            >
              ...
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.7rem' }}>
                ...
                <ChartMetricToggle
                  ...
                  label="Visitors"
                  onChange={setShowVisitors}
                />
                <ChartMetricToggle
                  checked={showSessions}
                  label="Sessions"
                  onChange={setShowSessions}
                />
              </div>
            </header>
        ...
        >
          <TopTable
            emptyLabel="No landing page rows returned for the selected timeframe."
            headers={['Landing Page', 'Sessions', 'Visitors']}
            heading="Top Landing Pages"
            rows={landingPageRows}
            subheading="Session entry pages for this window."
          />
          <TopTable
